/*!  Vue.js v2.6.3  * (c) 2014-2019 Evan You  */
...
/* SweetAlert.js https://unpkg.com/sweetalert/dist/sweetalert.min.js **/
...
var SwalAlert = function () {  
    return {
        Info: function (title, message) {   
            this._BaseAlert(title, message, 'info', 'Ok');
        },
        Success: function (title, message) {
            this._BaseAlert(title, message, 'success', 'Ok');
        },
        Warning: function (title, message) {
            this._BaseAlert(title, message, 'warning', 'Ok');
        },
        Danger: function (title, message) {
            this._BaseAlert(title, message, 'error', 'Ok');
        },   
        Error: function (title, message) {
            this._BaseAlert(title, message, 'error', 'Ok');
        },

        _BaseAlert: function (title, message, icon, buttonText) {
            title = title || "";
            message = message || "";
            if (message == "") {
                message = title;
...
function ButtonLoading(button) {
    var data_loading = "";
    try { data_loading = $(button).attr("data-loading"); } catch (e) { }
    if (data_loading == null || data_loading == "" || data_loading == "undefined") {
        data_loading = "loading";
    }

    data_loading = "<i class='fa fa-spinner fa-pulse fa-fw'></i> " + data_loading;
    $(button).attr("disabled", "disabled");   $(button).addClass("not-allowed");   $(button).html(data_loading);
}   
function ButtonRemoveLoading(button) {
    $(button).removeAttr("disabled");   $(button).removeClass("not-allowed");   $(button).html($(button).attr("after-loading"));
}       
function PageLoading() {   $("#submit-progress, #grayout").show();  }  
function PageRemoveLoading() { $("#submit-progress, #grayout").hide(); }
...
function WatermarkToMoney(control, hiddenControl) {
    let amount = 0;
...
function LoadDatePicker(datePickerControl) {
    let control = datePickerControl || ".datepicker";
    $(control).datepicker({
        format: 'dd/mm/yyyy', todayHighlight: true, endDate: new Date(), autoclose: true,
...
function SubmitSuspendClaim() {  
    var modelId = $("#ModelUnqkey").val();
    var reason = $("#Drp_SuspendReason").val() || "";
    if (reason.length < 3) {
        SwalAlert.Warning("Please select the reason for Suspension...");
        return;
    }
    if (reason == 'others') {
        reason = $.trim($("#Drp_SuspendReason_others").val());
        if (reason.length < 3) {
            SwalAlert.Warning("Please enter the other reasons for Suspension...");
            $("#Drp_SuspendReason_others").focus();
            return;
        }   
    }
    ...
    PageLoading();
    $("#modalSuspendClaim").modal('hide');
    $.get('/ClaimsAdminAjax/SuspendClaim', { id: modelId, remark: reason }, function (result) {
        PageRemoveLoading();

        if (result.isSuccess) {
            window.location.href = "index";
            SwalAlert.Success("Successfully Suspended Claim..");
        } else {
            SwalAlert.Info(result.message);
        }
    }).fail(function (error) {
        PageRemoveLoading();
        console.log(error);
...
function SubmitRejectClaim() {  
    var modelId = $("#ModelUnqkey").val();
    var reason = $("#Drp_RejectReason").val() || "";
    if (reason.length < 3) {
        SwalAlert.Warning("Please select the reason for Rejecting the Claim...");
        return;
    }
    if (reason == 'others') {
        reason = $.trim($("#Drp_RejectReason_others").val());
        if (reason.length < 3) {
            SwalAlert.Warning("Please enter the other reasons for Rejecting the Claim...");
            $("#Drp_RejectReason_others").focus();
            return;
        }
    }

    PageLoading();
    $("#modalRejectClaim").modal('hide');
    $.get('/ClaimsAdminAjax/RejectClaim', { id: modelId, remark: reason }, function (result) {
        PageRemoveLoading();

        if (result.isSuccess) {
            window.location.href = "index";
            SwalAlert.Success("Successfully Rejected Claim..");
        } else {
            SwalAlert.Info(result.message);
        }
    }).fail(function (error) {
...
function GetSuspendRejectCheck() {
    let suspendId = "#Drp_SuspendReason";
    let rejectId = "#Drp_RejectReason";
    _GetSuspendRejectReasons('CHECK', suspendId, rejectId);
}    
function GetSuspendRejectCode() {
    let suspendId = "#Drp_SuspendReason";
    let rejectId = "#Drp_RejectReason";
    _GetSuspendRejectReasons('CODE', suspendId, rejectId);
}      
function GetSuspendRejectClinical() {
    let suspendId = "#Drp_SuspendReason";
    let rejectId = "#ModalDrp_RejectReason,#Drp_RejectReason";
    _GetSuspendRejectReasons('CLINICAL', suspendId, rejectId);
}


function _GetSuspendRejectReasons(type, suspendControlId, rejectControlId) {
    //type=> CHECK, CODE, CLINICAL
    if ((suspendControlId || '') != '') $(suspendControlId).html("");
    if ((rejectControlId || '') != '') $(rejectControlId).html("");

    $.get('/ClaimsAdminAjax/GetSuspendRejectReasons', { type: type }, function (result) {
        //Json(new { SuspendReasons = listSuspends, RejectReasons = listRejects },
        try {
            let optS = '<option value="">Select One</option>';
            if ((suspendControlId || '') != '') {
                $.each(result.SuspendReasons, function (ind, obj) {
                    optS += '<option value="' + obj.item + '">' + obj.item + '</option>';
                });

                optS += '<option value="others">Others</option>';
                $(suspendControlId).html(optS);
            }
        } catch (x) { console.log(x); }

        try {
            let optR = '<option value="">Select One</option>';
            if ((rejectControlId || '') != '') {
                $.each(result.RejectReasons, function (ind, obj) {
                    optR += '<option value="' + obj.item + '">' + obj.item + '</option>';
                });

                optR += '<option value="others">Others</option>';
                $(rejectControlId).html(optR);
            }
        } catch (x) { console.log(x); }

        //>>>>>>>>>>>.
        $(suspendControlId).change(function () {
            let _valu = this.value;
            if (_valu == 'others') {
                $(suspendControlId + "_div").show(200);
            }
            else {
                $(suspendControlId + "_div").hide();
            }
        });
        $(rejectControlId).change(function () {
            let _valu = this.value;
            if (_valu == 'others') {
                $(rejectControlId + "_div").show(200);
            }
            else {
                $(rejectControlId + "_div").hide();
            }
        });
...
function GetBeneficiaryHcps(beneficiary, fxnResult) {
    $.get('/ClaimsAdminAjax/GetBeneficiaryHcp', { beneficiary: beneficiary }, function (result) {
        fxnResult(result);
    }).fail(function (error) {
        console.log(error);
    });
...
$(document).ready(function () {
    ...
    //document.getElementsByName("txt-depart-date")[0].setAttribute('min', today);
    //document.getElementsByName("txt-return-date")[0].setAttribute('min', today);


    setTimeout(function () {
        let beneficiary = $("#ModelBeneficiary").val() || "";
        if (beneficiary != "") {
            vueApp.BENEFICIARY_HCPS = [];
            GetBeneficiaryHcps(beneficiary, function (result) { vueApp.BENEFICIARY_HCPS = result; });
        }
    }, 3000);
...
var DB_SESSION_CLAIM_SERVICES = [];
function SessionClaimServices(fxnResult) {
    if (typeof (Storage) !== "undefined") {
        ...
        let data = localStorage.getItem("DB_SESSION_CLAIM_SERVICES") || "";
         
        if (data != "") {
            let list = JSON.parse(data) || [];
            if (list.OutPatients.length > 0) {
                isExist = 1;
                DB_SESSION_CLAIM_SERVICES = list;
                fxnResult(list);
                return;
            }   
        }
        ...
        if (isExist == 0) {
            PageLoading();
            $.get('/ClaimsAdminAjax/GetServiceCodes', { hcp: 0 }, function (result) {
                //{ InPatients,  OutPatients,  Consummables, Drugs }
                localStorage.setItem("DB_SESSION_CLAIM_SERVICES", JSON.stringify(result));
                DB_SESSION_CLAIM_SERVICES = result;
                PageRemoveLoading();
                fxnResult(result);
            }).fail(function (error) {
                PageRemoveLoading();
        ...
        //if (sessionStorage.DB_SESSION_CLAIM_SERVICES) { 
        //    let list = JSON.parse(sessionStorage.DB_SESSION_CLAIM_SERVICES) || [];
        //    if (list.length > 0) {
        //        DB_SESSION_CLAIM_SERVICES = list;
        //        fxnResult(list);
        //        console.log('service claims exist already...');
        //        return;
        //    }   
        //} 
        ...
        SwalAlert.Danger("Please use a Modern Browser...");
        fxnResult([]);   
    }
}               

var DB_SESSION_HCP_PROCEDURES = [];
function SessionProcedures(HcpId, fxnResult) {
    if (typeof (Storage) !== "undefined") {
        // Code for localStorage/sessionStorage.
        //localStorage.setItem("lastname", "Smith");  SET
        // localStorage.getItem("lastname");

        if (sessionStorage.DB_SESSION_HCP_PROCEDURES) {  
            let list = JSON.parse(sessionStorage.DB_SESSION_HCP_PROCEDURES) || [];
            if (list.length > 0) {
                fxnResult(list);
                return;
            }
        ...
        PageLoading();
        $.get('/ClaimsAdminAjax/GetServiceCodes', { hcp: HcpId }, function (result) {
            //{ InPatients,  OutPatients,  Consummables, Drugs }
            let list = result.InPatients.concat(result.OutPatients, result.Consummables, result.Drugs);
            sessionStorage.DB_SESSION_HCP_PROCEDURES = JSON.stringify(list);
            PageRemoveLoading();
            fxnResult(list);
        }).fail(function (error) {
            PageRemoveLoading();
...
function VUE_DATA_TABLE(vue_app, tableId) {
    try {
    ...
    vue_app.$nextTick(function () { DATA_TABLE(tableId); });
}

function DATA_TABLE(tableId) {
    $(tableId).DataTable({
        ...
        //},
        responsive: true,
        language: { search: "_INPUT_", "sSearch": "Filter: ", searchPlaceholder: "Filter records" },
        //--DataTableExport
        dom: 'lBfrtip',
...
function CompareDateValid(startDate, endDate) {
    ...
    let newStart = arrStart[2] + "-" + arrStart[1] + "-" + arrStart[0];
    let newEnd = arrEnd[2] + "-" + arrEnd[1] + "-" + arrEnd[0];
    //what if its yyy-mm-dd
    if (arrStart[0].length == 4) {
        newStart = arrStart[0] + "-" + arrStart[1] + "-" + arrStart[2];
    }
    if (arrEnd[0].length == 4) {
        newEnd = arrEnd[0] + "-" + arrEnd[1] + "-" + arrEnd[2];
    }

    let date1 = Date.parse(newStart);
    let date2 = Date.parse(newEnd);
...
function convertToDMY(yyyyMMdd) {
    try {
        yyyyMMdd = (yyyyMMdd || "");
        if (yyyyMMdd == "") {
            return "";
        ...
        yyyyMMdd = (yyyyMMdd.replace(/-/g, "/")).split('/');

        let ddMMyyyy = yyyyMMdd[2] + "-" + yyyyMMdd[1] + "-" + yyyyMMdd[0];   //yyyymmdd
        if (yyyyMMdd[0].length != 4) {
            ddMMyyyy = yyyyMMdd[0] + "-" + yyyyMMdd[1] + "-" + yyyyMMdd[2];
        }
        return ddMMyyyy;
    } catch (ex) {
        console.log('error convertToDMY:', yyyyMMdd, ex); 
    }
    return "";
}

function convertToYmd(ddMMyyyy) {
    try {
        console.log('convert ddMMyyyy: ', ddMMyyyy);
        ddMMyyyy = (ddMMyyyy || "");
        if (ddMMyyyy == "") {
            return "";
        }

        ddMMyyyy = (ddMMyyyy.replace(/-/g, "/")).split('/');

        let yyyyMMdd = ddMMyyyy[2] + "-" + ddMMyyyy[1] + "-" + ddMMyyyy[0];   //yyyymmdd
        if (ddMMyyyy[0].length == 4) {
            yyyyMMdd = ddMMyyyy[0] + "-" + ddMMyyyy[1] + "-" + ddMMyyyy[2];
        }

        return yyyyMMdd;
    } catch (ex) {
        console.log('error convertToYmd:', ddMMyyyy, ex);   
    }
